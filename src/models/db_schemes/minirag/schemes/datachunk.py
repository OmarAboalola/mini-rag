...
class DataChunk (SQLAlchemyBase):
    ...
    chunk_uuid=Column(UUID(as_uuid=True) ,default=uuid.uuid4 , unique=True , nullable=False) #generate a unique value and cannot be null

    chunk_text=Column(String,nullable=False)
    chunk_metadata=Column(JSONB,nullable=True)
    chunk_order=Column(Integer,nullable=False)
    
    #this chunk belongs to what project (table)--> (fk)
    ...
    #2 relationships as we have 2 foreign keys
